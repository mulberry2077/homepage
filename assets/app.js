...
const UI = {
    ...
    triggerGlitchEvent: () => {
        ...
        // 2. IMMEDIATE MASSIVE INSTRUCTION OVERLAY
        const instruction = document.createElement('div');
        instruction.style.cssText = `
            position: fixed; top: 0; left: 0; width: 100vw; height: 100vh;
            display: flex; flex-direction: column; justify-content: center; align-items: center;
            background: rgba(0,0,0,0.85); z-index: 99999; 
            color: red; font-family: 'Courier New', monospace; font-weight: bold;
            text-align: center; pointer-events: none;
        `;
        instruction.innerHTML = `
            <div style="font-size: 5rem; text-shadow: 0 0 20px red; margin-bottom: 20px;">SYSTEM FAILURE</div>
            <div style="font-size: 3rem; color: white;">TYPE "WAKE" TO REBOOT</div>
            <div style="font-size: 1.5rem; color: #aaa; margin-top: 20px;">KEYBOARD INPUT REQUIRED</div>
        `;
        document.body.appendChild(instruction);

        // 3. Start the permanent flash loop
        const flashLoop = () => {
            if (Math.random() > 0.8) {
                ...
                el.style.cssText = `
                    ...
                    z-index: 10000; pointer-events: none; mix-blend-mode: exclusion;
                `;
    ...
    showFinale: () => {
        ...
        typeWriter(textEl, finalMsg, () => {
            setTimeout(() => {
                ...
                btn.innerText = "思い出した、俺の名前はマルベリーだ";
                btn.onclick = () => {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (recruitForm) {
        recruitForm.addEventListener('submit', (e) => {
            ...
            if (jobType === 'engineer') {
                alert("ご応募ありがとうございます。サイト上部の「SANCTUARY」ロゴを7回連続でクリックすると、重大なシステムエラー（trouble name; 現実への回帰）が発生します。修理をお願いします。");
            } else {
